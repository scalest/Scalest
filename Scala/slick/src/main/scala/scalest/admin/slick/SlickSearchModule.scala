...
import scalest.admin.schema.FieldSchema
...
trait SlickSearchModule extends SlickModuleBase {
  ...
  object SearchParam {
    implicit def fieldSchema[T]: FieldSchema[SearchParam[T]] = instance[SearchParam[T]]("search-param")
    ...
    implicit def decoder[T: Decoder]: Decoder[SearchParam[T]] = Decoder.instance { c =>
      if (c.value.isNull) EmptyParam[T]().asRight
        ...
        c.get[Operations.Operation]("operation").flatMap {
          ...
          case Operations.Like =>
            c.get[String]("value").map(StringParam(_, Operations.Like)).asInstanceOf[Result[SearchParam[T]]]
    ...
    implicit def encoder[T: Encoder]: Encoder[SearchParam[T]] = Encoder.instance {
      ...
      case p: LogicalParam[T] =>
        obj(
          "operation" -> p.operation.asJson,
          "first"     -> p.first.asJson(encoder),
          "seconds"   -> p.second.asJson(encoder),
        )
  ...
  case class StringParam(value: String, op: Operations.Operation)
      extends SearchParam[String](op)
      with SingleParam[String] {
  ...
  case class AndParam[T](first: SearchParam[T], second: SearchParam[T])
      extends SearchParam[T](Operations.And)
      with LogicalParam[T] {
    def interpret(field: Rep[T])(implicit jdbcType: JdbcType[T]): Rep[Boolean] =
      first.interpret(field) && second.interpret(field)
  }

  case class OrParam[T](first: SearchParam[T], second: SearchParam[T])
      extends SearchParam[T](Operations.Or)
      with LogicalParam[T] {
    def interpret(field: Rep[T])(implicit jdbcType: JdbcType[T]): Rep[Boolean] =
      first.interpret(field) || second.interpret(field)
  }
