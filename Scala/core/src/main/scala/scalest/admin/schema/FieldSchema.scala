...
object FieldSchema {
  def instance[T](name: String,
                  default: Option[Json] = None,
                  addition: Option[Json] = None,
                  isOptional: Boolean = false): FieldSchema[T] = {
...
trait FieldTypeSchemaInstances {
  ...
  implicit def optionFTS[T](implicit elementFTS: FieldSchema[T]): FieldSchema[Option[T]] = elementFTS.copy(isOptional = true)
