...
package object error extends ErrorMessages {
  ...
  case class InternalServerError(error: String, details: Option[Json] = None) extends CommonError

  object InternalServerError {
    implicit val codec: Codec[InternalServerError] = deriveConfiguredCodec[InternalServerError]
  }

  case class ApplicationError(error: String, details: Option[Json] = None) extends CommonError

  object ApplicationError {
    implicit val codec: Codec[ApplicationError] = deriveConfiguredCodec[ApplicationError]
  }

  case class NotFoundError(error: String, details: Option[Json] = None) extends CommonError

  object NotFoundError {
    implicit val codec: Codec[NotFoundError] = deriveCodec[NotFoundError]
  }

  case class FieldValidationError(error: String, description: Option[String] = None)
