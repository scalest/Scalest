...
package object exception {
  ...
  abstract class RootException(error: CommonError, cause: Option[Throwable] = None) extends RuntimeException(error.error, cause.orNull)
  ...
  object ApplicationException {
    def apply(error: String, cause: Option[Throwable] = None): ApplicationException = new ApplicationException(ApplicationError(error), cause)
  }
  ...
  object DatabaseException {
    def apply(error: String, cause: Throwable): DatabaseException = new DatabaseException(InternalServerError(error), cause)
  }
