...
import pet.PetModel.{Genders, Pet, User}
...
object Migration {
  ...
  def randomUser(): User =
    User(id = UUID.randomUUID.toString, username = Random.nextString(10), password = Random.nextString(10))
    ...
    Await.ready(
      dc.db.run(
        DBIO.seq(
          ...
          Users.users.schema.createIfNotExists,
          Users.users ++= randomList(10, randomUser)
        )
      ),
      Duration.Inf
    )
