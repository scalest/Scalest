...
object FieldMacros {
  ...
  def childMacro[A: c.WeakTypeTag, B: c.WeakTypeTag](c: blackbox.Context)(function: c.Expr[A => B]): c.Expr[Field[B]] = {
    import c.universe._
    ...
    val parents = names.dropRight(1)
      .foldRight(field) { (n, f) =>
        q"""shay.common.Validation.Field($n, $f.value.${TermName(n)}, Some($f))"""
      }

    c.Expr[Field[B]](q"""shay.common.Validation.Field($resultName, $function($field.value), Some($parents))""")
  }
