...
package object validation {
  ...
  case class ValidationSchema(path: String, `type`: ValidationType, description: String)
  ...
  def validateEnum(field: Field[String], enum: Enumeration): ValidationResult =
    Try(enum.withName(field.value)).toOption
      .map(_ => Valid)
  ...
  def lift2[A, B](f: Field[A] => ValidationResult): Field[Option[A]] => ValidationResult = { fA =>
    fA.value.map(value => f(Field(fA.name, value, fA.parent))).getOrElse(().validNel)
  }

  def lift3[A, B, C](f: (Field[A], B) => ValidationResult): (Field[Option[A]], B) => ValidationResult = { (fA, b) =>
    fA.value.map(value => f(Field(fA.name, value, fA.parent), b)).getOrElse(().validNel)
  }
  ...
  def collectErrors(errors: NonEmptyList[FieldValidation]): ModelValidationError =
    errors.foldLeft(ModelValidationError(ValidationErrorMessage)) { (apiError, fieldValidation) =>
      apiError.copy(fields =
        apiError.fields + (fieldValidation.fieldName -> FieldValidationError(fieldValidation.fieldError)),
      )
    }
}
