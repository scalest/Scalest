...
case class HealthModule[F[_]](healthChecks: Seq[HealthCheck[F]] = Seq.empty)
                             (implicit timeout: Timeout[F], AE: ApplicativeError[F, Throwable]) extends TapirModule[F] {
  private val healthLogic = {
    healthChecks
      .map { h =>
        timeout
  ...
  override val routes: List[ServerEndpoint[_, _, _, Nothing, F]] = List(healthEndpoint)
}
