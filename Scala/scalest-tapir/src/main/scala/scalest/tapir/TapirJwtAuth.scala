...
trait TapirJwtAuth {
  ...
  def jwtClaimCodec[T](decode: String => Either[Throwable, T], encode: T => String): PlainCodec[T] =
    bearerCodec.mapDecode[T](decode(_).fold(Error(MalformedFieldErrorMessage, _), Value(_)))(encode)

  def jwtAuth[T](decode: String => Either[Throwable, T], encode: T => String): EndpointInput.Auth.Http[T] =
