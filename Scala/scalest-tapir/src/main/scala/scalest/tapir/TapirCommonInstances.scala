...
import sttp.tapir.{Codec, CodecForOptional, CodecFormat, CodecMeta, DecodeResult, EndpointIO, Schema, SchemaType, StringValueType, Validator}
...
trait TapirCommonInstances { self: TapirJsonCirce =>
  ...
  implicit def plainEnumCodec[E <: Enumeration#Value : WeakTypeTag]: PlainCodec[E] = {
    ...
    Codec.stringPlainCodecUtf8
      .mapDecode(s => Try(enum.withName(s).asInstanceOf[E]).fold(Error(MalformedFieldErrorMessage, _), Value(_)))(_.toString)
  ...
  def customCodec[T: Schema, C <: CodecFormat](format: C, parse: String => T): Codec[T, C, String] = new Codec[T, C, String] {
    ...
    override val meta: CodecMeta[T, C, String] = CodecMeta(implicitly, format, StringValueType(StandardCharsets.UTF_8))
  }
  def csvBody[T](implicit codec: CodecForOptional[T, TextCsv, _]): EndpointIO.Body[T, TextCsv, _] = EndpointIO.Body(codec, EndpointIO.Info.empty)
