...
trait TapirServerCommon { self: TapirJsonCirce =>
  ...
  val defaultHandler: DefaultDecodeFailureHandler =
    ServerDefaults.decodeFailureHandler.copy(response = defaultFailureResponse)
  ...
  val decodeFailureHandler: DecodeFailureHandler = ctx => {
    ctx.input match {
      case h: EndpointInput.Query[_] =>
        response(jsonBody[ModelValidationError])(
          ModelValidationError(ValidationErrorMessage, Map(h.name -> FieldValidationError(MalformedFieldErrorMessage))),
        )
